 ...
 * - Takes a string reference representing the password
...
bool isPasswordValid(string& password)
{
    ...
    return (password.length() >= 4) && (password.find_first_not_of(allowedChars) == string::npos);
}
		...
		vector<absSales*>& salesStaff)
{
    ...
    while (customerID.length() != 6 || !all_of(customerID.begin(), customerID.end(), ::isdigit))
    {
	...
	while (salespersonID.length() != 6 || !all_of(salespersonID.begin(), salespersonID.end(), ::isdigit))
	{
